...
def train_mann_transformer(model, dataset, num_epochs=50, batch_size=32, 
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, task in enumerate(few_shot_batches):
            ...
            # assuming `scaler` is in scope (rename your returned scaler to `scaler`)
            true_scaled = query_y.squeeze().cpu().numpy().reshape(-1,1)
            pred_scaled = query_pred.squeeze().detach().cpu().numpy().reshape(-1,1)
            true_raw   = scaler.inverse_transform(true_scaled).flatten().tolist()
            pred_raw   = scaler.inverse_transform(pred_scaled).flatten().tolist()
        ...
        # keep only t+1
                ...
                # in your validation loop, after you get `target` and `pred`:
                ts = target.cpu().numpy().reshape(-1,1)
                ps = pred.squeeze(0).cpu().numpy().reshape(-1,1)
                true_raw = scaler.inverse_transform(ts).flatten().tolist()
                pred_raw = scaler.inverse_transform(ps).flatten().tolist()
...
# then:
    ...
    with torch.no_grad():
        for features, target, _ in test_dataset:
            ...
            pred_scaled = pred.squeeze(0).detach().cpu().numpy().reshape(-1,1)
            ...
            true_vec_scaled = np.array(true_raw).reshape(-1,1)
            pred_vec_scaled = np.array(pred_raw).reshape(-1,1)
            all_true_vectors.append(scaler.inverse_transform(true_vec_scaled).flatten())
            all_pred_vectors.append(scaler.inverse_transform(pred_vec_scaled).flatten())
...
def prepare_discharge_data(file_path):
    ...
    scaler = MinMaxScaler()
    data['Simga_scaled'] = scaler.fit_transform(data[['Simga']])
